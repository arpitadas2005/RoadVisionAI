...
export const ErrorState: React.FC<ErrorStateProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center bg-white border border-slate-200/80 rounded-3xl my-4 shadow-sm">
      <div className="p-3 bg-red-50 text-red-600 rounded-2xl mb-3 border border-red-100 shadow-xs">
        <AlertCircle className="w-7 h-7" />
      </div>
      <h3 className="text-base font-bold text-slate-900 mb-1">{title}</h3>
      <p className="text-xs text-slate-500 font-medium max-w-sm mb-5 leading-relaxed">{message}</p>
      {onRetry && (
