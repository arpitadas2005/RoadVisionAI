...
export const HistoryTable: React.FC<HistoryTableProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Desktop Table View */}
      <div className="hidden md:block bg-slate-900/90 border border-slate-800 rounded-2xl overflow-hidden shadow-xl shadow-slate-950/40">
        ...
        {paginatedItems.map((item) => (
          <div key={item.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-800">
              <div className="flex flex-wrap gap-1">
                {item.detections.length === 0 ? (
                  <span className="text-emerald-400 text-[10px] font-semibold">Normal Surface</span>
                ) : (
              ...
              <div className="flex items-center gap-2">
                <Link
                  to={`/result/${item.id}`}
                  className="px-2.5 py-1.5 rounded-lg bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 text-xs font-bold flex items-center gap-1"
                >
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="flex items-center justify-between p-4 bg-slate-900/90 border border-slate-800 rounded-2xl text-xs">
          <div className="text-slate-400">
            Showing <strong className="text-slate-200">{startIndex + 1}</strong> to{' '}
            <strong className="text-slate-200">{Math.min(startIndex + pageSize, history.length)}</strong> of{' '}
            <strong className="text-slate-200">{history.length}</strong> inspections
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-3 py-1 bg-slate-950 border border-slate-800 rounded-lg text-slate-200 font-mono text-xs font-bold">
